...
class Atendimento {
  ...
  adiciona(item) {
    ...
    SELECT * FROM Clientes WHERE Clientes.id = ${clienteId}; SELECT * FROM Pets WHERE Pets.id = ${petId};
    SELECT * FROM Servicos WHERE Serivcos.id = ${servicoId}`
    ...
    return executaQuery(sql).then(resposta => {
      ...
      const pet = repsosta[2][0]

      const servico = resposta[3][0]

      return ({ id: dados.insertId, cliente, pet, servico, data, status, observacoes })
